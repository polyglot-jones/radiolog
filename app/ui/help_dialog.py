import logging
from app.logic.status_codes import StatusCode, STATUS_STYLE_DICT
...
class HelpDialog(GuiBaseClass, GuiSpec):
    ...
    def stylize(self):
        ...
        self.colorLabel1.setStyleSheet(STATUS_STYLE_DICT[StatusCode.AT_IC])
        self.colorLabel2.setStyleSheet(STATUS_STYLE_DICT[StatusCode.IN_TRANSIT])
        self.colorLabel3.setStyleSheet(STATUS_STYLE_DICT[StatusCode.WORKING])
        self.colorLabel4.setStyleSheet(STATUS_STYLE_DICT[StatusCode.WAITING_FOR_TRANSPORT])
        self.colorLabel5.setStyleSheet(STATUS_STYLE_DICT[StatusCode.TIMED_OUT_ORANGE])
        self.colorLabel6.setStyleSheet(STATUS_STYLE_DICT[StatusCode.TIMED_OUT_RED])

        self.fsSomeFilteredLabel.setFont(HELP_FONT)
        self.fsAllFilteredLabel.setFont(HELP_FONT_STRIKEOUT)
        self.fsSomeFilteredLabel.setStyleSheet(STATUS_STYLE_DICT[StatusCode.WORKING])
        self.fsAllFilteredLabel.setStyleSheet(STATUS_STYLE_DICT[StatusCode.WORKING])

    def update_blinking(self, blinkToggle, STATUS_STYLE_DICT):

        if blinkToggle == 1:
            self.colorLabel4.setStyleSheet(STATUS_STYLE_DICT[StatusCode.CALL_ATTENTION])
            self.colorLabel5.setStyleSheet(STATUS_STYLE_DICT[StatusCode.TIMED_OUT_ORANGE])
            self.colorLabel6.setStyleSheet(STATUS_STYLE_DICT[StatusCode.TIMED_OUT_RED])
            self.colorLabel7.setStyleSheet(STATUS_STYLE_DICT[StatusCode.CALL_ATTENTION])
            self.fsSomeFilteredLabel.setFont(HELP_FONT_STRIKEOUT)
        else:
            self.colorLabel4.setStyleSheet(STATUS_STYLE_DICT[StatusCode.WAITING_FOR_TRANSPORT])
            self.colorLabel5.setStyleSheet(STATUS_STYLE_DICT[StatusCode.CALL_ATTENTION])
            self.colorLabel6.setStyleSheet(STATUS_STYLE_DICT[StatusCode.CALL_ATTENTION])
            self.colorLabel7.setStyleSheet(STATUS_STYLE_DICT[StatusCode.STANDBY])
            self.fsSomeFilteredLabel.setFont(HELP_FONT)
